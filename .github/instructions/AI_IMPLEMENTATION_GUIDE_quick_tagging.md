...
## 🎯 Feature Overview
**Feature ID:** `logging.quick_tagging`
**Title:** Quick Tagging Chips
**Epic:** logging (LOGGING) | **Priority:** P1 | **Order:** 4
...
You are tasked with implementing **Quick Tagging Chips** (logging.quick_tagging) as part of the AshTrail mobile application.
...
Reduce friction adding frequent tags to a log at capture time.
...
test/features/quick_tagging/         # Comprehensive test coverage
...
### ✅ Acceptance Criteria
  - **Must:** Top 5 most recent or frequent tags appear as selectable chips above record button.
  - **Must:** Ability to add from infrequent tags list.
  - **Must:** Selected tags persisted via SmokeLogTag writes with same ts as SmokeLog.
  - **Must:** Works offline and queues tag creates.

### 👤 User Stories
  - As a user, As a user I tap one or more recent/frequent tags before pressing the hold button.
...
#### 1.1 Define Entities
```dart
// lib/features/quick_tagging/domain/entities/quick_tagging_entity.dart
import 'package:freezed_annotation/freezed_annotation.dart';

part 'quick_tagging_entity.freezed.dart';

@freezed
class QuickTaggingEntity with _$QuickTaggingEntity {
  const factory QuickTaggingEntity({
    required String id,
...
```dart
// lib/features/quick_tagging/domain/repositories/quick_tagging_repository.dart
import 'package:dartz/dartz.dart';
import '../entities/quick_tagging_entity.dart';
import '../../../../core/error/failures.dart';

abstract class QuickTaggingRepository {
  Future<Either<AppFailure, List<QuickTaggingEntity>>> getAll();
  Future<Either<AppFailure, QuickTaggingEntity>> getById(String id);
  Future<Either<AppFailure, QuickTaggingEntity>> create(QuickTaggingEntity entity);
  Future<Either<AppFailure, QuickTaggingEntity>> update(QuickTaggingEntity entity);
...
```dart
// lib/features/quick_tagging/domain/usecases/get_quick_taggings_usecase.dart
import 'package:dartz/dartz.dart';
...
@injectable
class GetQuickTaggingsUseCase implements UseCase<List<QuickTaggingEntity>, NoParams> {
  const GetQuickTaggingsUseCase(this._repository);
  
  final QuickTaggingRepository _repository;
  
  @override
  Future<Either<AppFailure, List<QuickTaggingEntity>>> call(NoParams params) {
...
```dart
// lib/features/quick_tagging/data/models/quick_tagging_model.dart
import 'package:freezed_annotation/freezed_annotation.dart';
import 'package:isar/isar.dart';
import '../../domain/entities/quick_tagging_entity.dart';

part 'quick_tagging_model.freezed.dart';
part 'quick_tagging_model.g.dart';

@freezed
@Collection()
class QuickTaggingModel with _$QuickTaggingModel {
  const factory QuickTaggingModel({
    @JsonKey(name: 'id') required String id,
    @JsonKey(name: 'created_at') required DateTime createdAt,
    @JsonKey(name: 'updated_at') DateTime? updatedAt,
  }) = _QuickTaggingModel;
  
  factory QuickTaggingModel.fromJson(Map<String, dynamic> json) =>
      _$QuickTaggingModelFromJson(json);
  
  QuickTaggingEntity toEntity() => QuickTaggingEntity(
    id: id,
    createdAt: createdAt,
    updatedAt: updatedAt,
  );
  
  factory QuickTaggingModel.fromEntity(QuickTaggingEntity entity) =>
      QuickTaggingModel(
        id: entity.id,
...
```dart
// lib/features/quick_tagging/presentation/providers/quick_tagging_providers.dart
import 'package:riverpod_annotation/riverpod_annotation.dart';
import '../../domain/entities/quick_tagging_entity.dart';
import '../../domain/usecases/get_quick_taggings_usecase.dart';
import '../../../../core/usecases/usecase.dart';
import '../../../../core/di/injection.dart';

part 'quick_tagging_providers.g.dart';

@riverpod
class QuickTaggingListNotifier extends _QuickTaggingListNotifier {
  @override
  Future<List<QuickTaggingEntity>> build() async {
    final useCase = getIt<GetQuickTaggingsUseCase>();
...
- Target ≥85% overall coverage for new `quick_tagging` feature code
...
#### Unit Tests
```dart
// test/features/quick_tagging/domain/usecases/get_quick_taggings_usecase_test.dart
import 'package:dartz/dartz.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mocktail/mocktail.dart';

void main() {
  group('GetQuickTaggingsUseCase', () {
...
#### Widget Tests
```dart
// test/features/quick_tagging/presentation/screens/quick_tagging_screen_test.dart
...
void main() {
  testWidgets('displays loading state correctly', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          quick_taggingListNotifierProvider.overrideWith((ref) {
            return const AsyncValue.loading();
          }),
        ],
        child: const MaterialApp(home: QuickTaggingScreen()),
      ),
    );
...
flutter test --coverage test/features/quick_tagging/
...
python scripts/dev_assistant.py finalize-feature --feature-id logging.quick_tagging --dry-run
```
...
**Generated:** 2025-09-20T21:35:16.120672+00:00
**Complexity:** Moderate | **Epic:** logging | **Priority:** P1
