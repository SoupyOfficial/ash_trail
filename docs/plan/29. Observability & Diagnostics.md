...
#### 29.2.2 Debug Views (Internal Screens)
...
#### 29.2.4 Privacy Constraints (Data Redaction)
- Prevent leaking:
...
### 29.3 Data structures (fields, types, constraints)
...
#### 29.3.4 Redaction configuration
...
### 29.4 State & flow (how data moves through the system)

#### 29.4.1 Usage metrics flow
...
#### 29.4.2 Debug views flow (internal screens)
...
#### 29.4.3 User-reported issues flow (log bundles)
...
### 29.6 Future extensions (clearly labeled)
...
### Logging Library (Answered)
- **Logging mechanism**: Ash Trail uses the [logger](https://pub.dev/packages/logger) package with a centralized `AppLogger` wrapper (`lib/logging/app_logger.dart`). Logs are structured by component name and level (trace, debug, info, warning, error, fatal). In release builds, only warning and above are emitted. See `docs/LOGGING_CONVENTIONS.md` for usage guidelines.
