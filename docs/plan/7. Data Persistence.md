...
##### MVP Status: Local Hive Only

The current MVP implementation uses Hive for all persistence (local only).  
Firestore integration is planned for post-MVP cross-device sync.

##### Future: Firestore Collection Structure

When Firestore is integrated, data will be namespaced per account:
...
- Account ID will be always embedded in the path
- Security rules will enforce account isolation

##### Current Implementation

All data is stored locally in Hive boxes:
- Accounts stored in `accounts` box
- Logs stored in `logs` box (keyed by `{accountId}:{logId}` for isolation)
