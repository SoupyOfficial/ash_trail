...
## 🎯 Feature Overview
**Feature ID:** `ui.reachability`
**Title:** Reachability & Ergonomics Audit
**Epic:** ui (UI) | **Priority:** P1 | **Order:** 14
...
You are tasked with implementing **Reachability & Ergonomics Audit** (ui.reachability) as part of the AshTrail mobile application.
...
test/features/reachability/         # Comprehensive test coverage
...
### ✅ Acceptance Criteria
  - **Must:** Primary frequent actions remain in lower 60% vertical region portrait.
  - **Must:** Audit document produced with thumb zone heatmap for baseline device.
  - **Must:** Any action outside zone has alternate accessible trigger.
...
#### 1.1 Define Entities
```dart
// lib/features/reachability/domain/entities/reachability_entity.dart
import 'package:freezed_annotation/freezed_annotation.dart';

part 'reachability_entity.freezed.dart';

@freezed
class ReachabilityEntity with _$ReachabilityEntity {
  const factory ReachabilityEntity({
    required String id,
...
```dart
// lib/features/reachability/domain/repositories/reachability_repository.dart
import 'package:dartz/dartz.dart';
import '../entities/reachability_entity.dart';
import '../../../../core/error/failures.dart';

abstract class ReachabilityRepository {
  Future<Either<AppFailure, List<ReachabilityEntity>>> getAll();
  Future<Either<AppFailure, ReachabilityEntity>> getById(String id);
  Future<Either<AppFailure, ReachabilityEntity>> create(ReachabilityEntity entity);
  Future<Either<AppFailure, ReachabilityEntity>> update(ReachabilityEntity entity);
...
```dart
// lib/features/reachability/domain/usecases/get_reachabilitys_usecase.dart
import 'package:dartz/dartz.dart';
...
@injectable
class GetReachabilitysUseCase implements UseCase<List<ReachabilityEntity>, NoParams> {
  const GetReachabilitysUseCase(this._repository);
  
  final ReachabilityRepository _repository;
  
  @override
  Future<Either<AppFailure, List<ReachabilityEntity>>> call(NoParams params) {
...
```dart
// lib/features/reachability/data/models/reachability_model.dart
import 'package:freezed_annotation/freezed_annotation.dart';
import 'package:isar/isar.dart';
import '../../domain/entities/reachability_entity.dart';

part 'reachability_model.freezed.dart';
part 'reachability_model.g.dart';

@freezed
@Collection()
class ReachabilityModel with _$ReachabilityModel {
  const factory ReachabilityModel({
    @JsonKey(name: 'id') required String id,
    @JsonKey(name: 'created_at') required DateTime createdAt,
    @JsonKey(name: 'updated_at') DateTime? updatedAt,
  }) = _ReachabilityModel;
  
  factory ReachabilityModel.fromJson(Map<String, dynamic> json) =>
      _$ReachabilityModelFromJson(json);
  
  ReachabilityEntity toEntity() => ReachabilityEntity(
    id: id,
    createdAt: createdAt,
    updatedAt: updatedAt,
  );
  
  factory ReachabilityModel.fromEntity(ReachabilityEntity entity) =>
      ReachabilityModel(
        id: entity.id,
...
```dart
// lib/features/reachability/presentation/providers/reachability_providers.dart
import 'package:riverpod_annotation/riverpod_annotation.dart';
import '../../domain/entities/reachability_entity.dart';
import '../../domain/usecases/get_reachabilitys_usecase.dart';
import '../../../../core/usecases/usecase.dart';
...
@riverpod
class ReachabilityListNotifier extends _ReachabilityListNotifier {
  @override
  Future<List<ReachabilityEntity>> build() async {
    final useCase = getIt<GetReachabilitysUseCase>();
...
- Target ≥85% overall coverage for new `reachability` feature code
...
#### Unit Tests
```dart
// test/features/reachability/domain/usecases/get_reachabilitys_usecase_test.dart
import 'package:dartz/dartz.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mocktail/mocktail.dart';

void main() {
  group('GetReachabilitysUseCase', () {
...
#### Widget Tests
```dart
// test/features/reachability/presentation/screens/reachability_screen_test.dart
...
void main() {
  testWidgets('displays loading state correctly', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          reachabilityListNotifierProvider.overrideWith((ref) {
            return const AsyncValue.loading();
          }),
        ],
        child: const MaterialApp(home: ReachabilityScreen()),
      ),
    );
...
flutter test --coverage test/features/reachability/
...
python scripts/dev_assistant.py finalize-feature --feature-id ui.reachability --dry-run
```
...
**Generated:** 2025-09-18T01:00:45.482250+00:00
