...
## 🎯 Feature Overview
**Feature ID:** `ui.spotlight_indexing`
**Title:** Spotlight Indexing
**Epic:** ui (UI) | **Priority:** P3 | **Order:** 13
...
You are tasked with implementing **Spotlight Indexing** (ui.spotlight_indexing) as part of the AshTrail mobile application.
...
```
lib/features/spotlight_indexing/
...
test/features/spotlight_indexing/         # Comprehensive test coverage
...
### ✅ Acceptance Criteria
  - **Must:** Recent tags and saved chart views indexed; selecting result deep links correctly.
  - **Must:** Deindex removed / renamed items within one sync cycle.
...
#### 1.1 Define Entities
```dart
// lib/features/spotlight_indexing/domain/entities/spotlight_indexing_entity.dart
import 'package:freezed_annotation/freezed_annotation.dart';

part 'spotlight_indexing_entity.freezed.dart';

@freezed
class SpotlightIndexingEntity with _$SpotlightIndexingEntity {
  const factory SpotlightIndexingEntity({
    required String id,
    required DateTime createdAt,
    DateTime? updatedAt,
    // Add your domain fields here based on requirements
  }) = _SpotlightIndexingEntity;
...
```dart
// lib/features/spotlight_indexing/domain/repositories/spotlight_indexing_repository.dart
import 'package:dartz/dartz.dart';
import '../entities/spotlight_indexing_entity.dart';
import '../../../../core/error/failures.dart';

abstract class SpotlightIndexingRepository {
  Future<Either<AppFailure, List<SpotlightIndexingEntity>>> getAll();
  Future<Either<AppFailure, SpotlightIndexingEntity>> getById(String id);
  Future<Either<AppFailure, SpotlightIndexingEntity>> create(SpotlightIndexingEntity entity);
  Future<Either<AppFailure, SpotlightIndexingEntity>> update(SpotlightIndexingEntity entity);
...
```dart
// lib/features/spotlight_indexing/domain/usecases/get_spotlight_indexings_usecase.dart
import 'package:dartz/dartz.dart';
...
@injectable
class GetSpotlightIndexingsUseCase implements UseCase<List<SpotlightIndexingEntity>, NoParams> {
  const GetSpotlightIndexingsUseCase(this._repository);
  
  final SpotlightIndexingRepository _repository;
  
  @override
  Future<Either<AppFailure, List<SpotlightIndexingEntity>>> call(NoParams params) {
...
```dart
// lib/features/spotlight_indexing/data/models/spotlight_indexing_model.dart
import 'package:freezed_annotation/freezed_annotation.dart';
import 'package:isar/isar.dart';
import '../../domain/entities/spotlight_indexing_entity.dart';

part 'spotlight_indexing_model.freezed.dart';
part 'spotlight_indexing_model.g.dart';

@freezed
@Collection()
class SpotlightIndexingModel with _$SpotlightIndexingModel {
  const factory SpotlightIndexingModel({
    @JsonKey(name: 'id') required String id,
    @JsonKey(name: 'created_at') required DateTime createdAt,
    @JsonKey(name: 'updated_at') DateTime? updatedAt,
  }) = _SpotlightIndexingModel;
  
  factory SpotlightIndexingModel.fromJson(Map<String, dynamic> json) =>
      _$SpotlightIndexingModelFromJson(json);
  
  SpotlightIndexingEntity toEntity() => SpotlightIndexingEntity(
    id: id,
    createdAt: createdAt,
    updatedAt: updatedAt,
  );
  
  factory SpotlightIndexingModel.fromEntity(SpotlightIndexingEntity entity) =>
      SpotlightIndexingModel(
        id: entity.id,
...
```dart
// lib/features/spotlight_indexing/presentation/providers/spotlight_indexing_providers.dart
import 'package:riverpod_annotation/riverpod_annotation.dart';
import '../../domain/entities/spotlight_indexing_entity.dart';
import '../../domain/usecases/get_spotlight_indexings_usecase.dart';
import '../../../../core/usecases/usecase.dart';
...
@riverpod
class SpotlightIndexingListNotifier extends _SpotlightIndexingListNotifier {
  @override
  Future<List<SpotlightIndexingEntity>> build() async {
    final useCase = getIt<GetSpotlightIndexingsUseCase>();
...
- Target ≥85% overall coverage for new `spotlight_indexing` feature code
...
#### Unit Tests
```dart
// test/features/spotlight_indexing/domain/usecases/get_spotlight_indexings_usecase_test.dart
import 'package:dartz/dartz.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mocktail/mocktail.dart';

void main() {
  group('GetSpotlightIndexingsUseCase', () {
...
#### Widget Tests
```dart
// test/features/spotlight_indexing/presentation/screens/spotlight_indexing_screen_test.dart
...
void main() {
  testWidgets('displays loading state correctly', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          spotlight_indexingListNotifierProvider.overrideWith((ref) {
            return const AsyncValue.loading();
          }),
        ],
        child: const MaterialApp(home: SpotlightIndexingScreen()),
      ),
    );
...
flutter test --coverage test/features/spotlight_indexing/
...
python scripts/dev_assistant.py finalize-feature --feature-id ui.spotlight_indexing --dry-run
```
...
**Generated:** 2025-09-17T23:52:15.619948+00:00
