...
class HomeWidgetBuilder extends ConsumerWidget {
  ...
  Widget _buildHits(
    ...
    // For the "today" case, keep the live-updating card
    if (days == 1 && !hasEventFilter) {
      return _TotalHitsTodayCard(records: records, trendPeriod: trendPeriod);
    }
...
/// Card showing today's total hits up to current time,
/// with hour-block trend. Rebuilds every minute to update the time label.
class _TotalHitsTodayCard extends StatefulWidget {
  final List<LogRecord> records;
  final TrendComparisonPeriod trendPeriod;

  const _TotalHitsTodayCard({required this.records, required this.trendPeriod});

  @override
  State<_TotalHitsTodayCard> createState() => _TotalHitsTodayCardState();
}

class _TotalHitsTodayCardState extends State<_TotalHitsTodayCard> {
  late DateTime _asOf;
  Timer? _timer;
  ...
  Widget build(BuildContext context) {
    ...
    final compHitCount = metrics.getHitCount(compRecords).toDouble();
    final trend = metrics.computeHourBlockTrend(
      actualSoFar: data.count.toDouble(),
      fullDayReference: compHitCount,
      period: widget.trendPeriod,
      asOf: asOf,
    );
