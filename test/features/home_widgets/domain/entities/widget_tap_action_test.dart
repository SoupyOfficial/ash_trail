  ...
  group('WidgetTapAction Enum Tests', () {
    group('Constructor and Properties', () {
      test('should have correct display names for all actions', () {
        ...
        expect(WidgetTapAction.recordOverlay.displayName, equals('Record Overlay'));
    ...
    group('Deep Link Paths', () {
      ...
      test('should return correct deep link path for quickRecord', () {
        expect(WidgetTapAction.quickRecord.deepLinkPath, equals('/record?quick=true'));
      });

      test('should have unique deep link paths for all actions', () {
        final paths = WidgetTapAction.values.map((action) => action.deepLinkPath).toSet();
    ...
    group('Equality and Hash Code', () {
      test('should be equal to itself', () {
        ...
        expect(WidgetTapAction.recordOverlay, equals(WidgetTapAction.recordOverlay));
        expect(WidgetTapAction.viewLogs, equals(WidgetTapAction.viewLogs));
        expect(WidgetTapAction.quickRecord, equals(WidgetTapAction.quickRecord));
      });

      test('should have consistent hash codes', () {
        expect(WidgetTapAction.openApp.hashCode, equals(WidgetTapAction.openApp.hashCode));
        expect(WidgetTapAction.recordOverlay.hashCode, equals(WidgetTapAction.recordOverlay.hashCode));
        expect(WidgetTapAction.viewLogs.hashCode, equals(WidgetTapAction.viewLogs.hashCode));
        expect(WidgetTapAction.quickRecord.hashCode, equals(WidgetTapAction.quickRecord.hashCode));
      });

      test('should not be equal to different enum values', () {
        expect(WidgetTapAction.openApp, isNot(equals(WidgetTapAction.recordOverlay)));
        expect(WidgetTapAction.recordOverlay, isNot(equals(WidgetTapAction.viewLogs)));
        expect(WidgetTapAction.viewLogs, isNot(equals(WidgetTapAction.quickRecord)));
      });
    });

    group('String Representation', () {
      test('should have meaningful string representation', () {
        expect(WidgetTapAction.openApp.toString(), equals('WidgetTapAction.openApp'));
        expect(WidgetTapAction.recordOverlay.toString(), equals('WidgetTapAction.recordOverlay'));
        expect(WidgetTapAction.viewLogs.toString(), equals('WidgetTapAction.viewLogs'));
        expect(WidgetTapAction.quickRecord.toString(), equals('WidgetTapAction.quickRecord'));
      });
    });
    ...
    group('Business Logic Edge Cases', () {
      ...
      test('should have display names for all actions', () {
        for (final action in WidgetTapAction.values) {
          expect(action.displayName.isNotEmpty, isTrue, reason: 'Action $action should have a display name');
        }
      });
    });
  });
