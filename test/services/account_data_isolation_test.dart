...
void main() {
  ...
  group('Account Data Isolation - Integration with Real Database', () {
    /// These tests use the real Hive database for integration testing.
    /// They verify that account isolation works at the persistence layer.
    /// Note: Skipped in unit test environment - requires full platform setup.

    test(
      'real database maintains account isolation',
      skip: 'Requires platform plugin setup',
      () async {
        ...
        try {
          ...
          // WHEN: Querying each account's logs
          final accountALogs = await service.getLogRecords(accountId: accountA);
          final accountBLogs = await service.getLogRecords(accountId: accountB);

          // THEN: Each account should only see their own log
          expect(accountALogs.length, 1);
